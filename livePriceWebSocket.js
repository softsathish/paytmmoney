	...
	parseBinary(packet) {
		...
		function processLtpPacket() {
			response.push({
				...
				last_trade_time: epochConverterUtil.EpochConverter(
					dvu.getInt32(position + 4, true)
				),
		...
		function processIndexLtpPacket() {
			response.push({
				last_price: dvu.getFloat32(position, true).toFixed(2),
				last_update_time: epochConverterUtil.EpochConverter(
					dvu.getInt32(position + 4, true)
				),
				security_id: dvu.getInt32(position + 8, true),
				tradable: dvu.getInt8(position + 12, true),
		...
		function processFullPacket() {
			...
			for (let i = 0; i < 5; i++) {
				...
				depthObj.sell_quantity = dvu.getInt32(
					position + 4 + i * depth_size,
					true
				);
				depthObj.buy_order = dvu.getInt16(position + 8 + i * depth_size, true);
				depthObj.sell_order = dvu.getInt16(
					position + 10 + i * depth_size,
					true
				);
				depthObj.buy_price = dvu
					.getFloat32(position + 12 + i * depth_size, true)
					.toFixed(2);
				depthObj.sell_price = dvu
					.getFloat32(position + 16 + i * depth_size, true)
					.toFixed(2);
			...
			(tick.last_price = dvu.getFloat32(position, true).toFixed(2)),
				(tick.last_trade_time = epochConverterUtil.EpochConverter(
					dvu.getInt32(position + 4, true)
				)),
				(tick.security_id = dvu.getInt32(position + 8, true)),
				(tick.name = dvu.getInt32(position + 8, true)),
				...
				(tick.average_traded_price = dvu
					.getFloat32(position + 18, true)
					.toFixed(2)),
				...
				(tick.fifty_two_week_high = dvu
					.getFloat32(position + 58, true)
					.toFixed(2)),
				(tick.fifty_two_week_low = dvu
					.getFloat32(position + 62, true)
					.toFixed(2)),
